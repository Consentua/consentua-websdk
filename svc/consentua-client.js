...
public function ConsentuaClient(clientID, serviceID, serviceKey, lang){
    ...
    self.login = function(){
        ...
        var args = {
                ClientId: clientID,
                ServiceId: serviceID,
                LoginToken: md5(serviceKey + '_' + (d.getUTCDate()) + '_' + (d.getUTCMonth() + 1) + '_' + (d.getUTCFullYear()))
        };

        self.postBody('/login/Service', args, false).done(loggedin);

        return deferred;
    }

    /**
     * Send a request to the API. You probably need to login() before doing so.
     */
    self.get = function(path, extra)
    {
        if(typeof extra != 'object' || extra === null) // typeof null === 'object' :/
            var extra = {};
        ...
        return $.get(baseurl + path, extra, null, 'json');
    }
    ...
    self.post = function(path, data)
    {
        ...
        return $.post(baseurl + path, extra, null, 'json');
    }
